...
var MenuManager = new (function()
{
	...
	{
		Lesktop.Desktop.Popup(menu.GetDom(), clientX, clientY);
	...
	this.Hide = function(menu)
	{
		...
		{
			menu.Hide();
			Lesktop.Desktop.Unpopup(menu.GetDom());
...
function SingleMenu(menuConfig, parentItem, mainMenu)
{
	...
	This.Show = function(clientX, clientY)
	{
		...
		if (x + obj.offsetWidth > Lesktop.Desktop.GetWidth())
		{
			x -= obj.offsetWidth;
			if (parentItem != null) x -= parentItem.GetDom().offsetWidth;
		}
		if (y + obj.offsetHeight > Lesktop.Desktop.GetHeight())
		{
			y -= obj.offsetHeight;
