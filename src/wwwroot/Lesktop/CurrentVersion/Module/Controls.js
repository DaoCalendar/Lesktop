...
function Control(config)
{
	...
	var _borderWidth = Lesktop.Utility.IsNull(config.BorderWidth, 0);
	This.GetBorderWidth = function() { return _borderWidth; }

	var _padding = Lesktop.Utility.IsNull(config.Padding, 0);
	This.GetPadding = function() { return _padding; }

	//框架
	var _dom = Lesktop.Utility.IsNull(config.Dom, document.createElement("DIV"));
	...
	This.OnResized = new Lesktop.Delegate();
	...
	//AnchorStyle
	var _anchorStyle = Lesktop.Utility.IsNull(config.AnchorStyle, AnchorStyle.Left | AnchorStyle.Top);
	...
	This.OnDispose = new Lesktop.Delegate();
	...
	Lesktop.Utility.AttachEvent(
		...
		function(evt)
		{
			...
			if (Lesktop.Utility.GetButton(evt) == "Right")
			{
	...
	//初始化位置和大小
	This.Move(Lesktop.Utility.IsNull(config.Left, 0), Lesktop.Utility.IsNull(config.Top, 0));
	This.Resize(Lesktop.Utility.IsNull(config.Width, 200), Lesktop.Utility.IsNull(config.Height, 150));
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "control"));
	This.SetParent(Lesktop.Utility.IsNull(config.Parent, null));
	This.SetVisible(Lesktop.Utility.IsNull(config.Visible, true));
}
...
function Label(config)
{
	...
	{
		This.GetDom().innerHTML = Lesktop.Utility.ReplaceHtml(value);
	}
	...
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "label"));
}
...
function Button(config)
{
	...
	This.OnClick = new Lesktop.Delegate();
	...
	'</tr>'+
	'</table>'
	_dom.tabIndex = Lesktop.Utility.IsNull(config.TabIndex, -1);
	...
	Lesktop.Utility.AttachEvent(
		...
		{
			if(evt==undefined) evt=window.event;
			if(Lesktop.Utility.GetButton(evt)=="")
			{
	...
	This.SetText = function(value)
	{
		This.GetDom().firstChild.rows[0].cells[1].innerHTML = Lesktop.Utility.ReplaceHtml(value);
		Base.SetText(value);
	}
	...
	Lesktop.Utility.DisableSelect(_dom,true);

	_dom = null;

	This.SetText(Lesktop.Utility.IsNull(config.Text, ""));
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "button"));
...
function TextBox(config)
{
	...
	This.SetText(Lesktop.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "textbox"));
}
...
function TextArea(config)
{
	...
	This.SetText(Lesktop.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "textarea"));
}
...
function CheckBox(config)
{
	...
	This.SetText = function(value)
	{
		_textdom.innerHTML = Lesktop.Utility.ReplaceHtml(value);
	}
	...
	This.SetText(Lesktop.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "checkbox"));
}
...
function RadioButton(config)
{
	...
	This.SetText(Lesktop.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "radio"));
}
...
function DropDownList(config)
{
	...
	This.SetText(Lesktop.Utility.IsNull(config.Text, ""));
	This.Resize(This.GetWidth(), This.GetHeight());
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "select"));
}
...
function ListBox(config)
{
	...
	This.GetDropDownListDom().size = Lesktop.Utility.IsNull(config.Size, 4);
...
function TreeView(config)
{
	...
	var nodeHeight = Lesktop.Utility.IsNull(config.NodeHeight, 20);
	var hasImage = Lesktop.Utility.IsNull(config.HasImage, false);

	var dom = This.GetDom();
	dom.innerHTML = "<div style='overflow:visible;'></div>";

	This.OnDblClick = new Lesktop.Delegate();
	This.OnClick = new Lesktop.Delegate();
	This.OnExpand = new Lesktop.Delegate();
	This.OnCollapse = new Lesktop.Delegate();
	This.OnBeginRequest = new Lesktop.Delegate();
	This.OnEndRequest = new Lesktop.Delegate();
	...
	*/
	This.Refresh = function(callback)
	{
				...
				function(subNodeData, error)
				{
					if (subNodeData != null)
					{
						...
						for (var i in subNodeData)
						{
							...
							var node = new TreeNode(This, null, nd.Name, nd.Text, nd.ImageCss, nd.Tag, dom.firstChild, dataSource, nodeHeight, i == subNodeData.length - 1, nd.HasChildren,nd.ImageSrc, nd.HasCheckBox);
							rootNodes.push(node);
		...
		{
			callback(null, new Lesktop.Exception(ex.name, ex.message));
		}
	}
	...
	*/
	This.Find = function(callback, path)
	{
		...
		{
			callback(null, new Lesktop.Exception("Error", "路径格式不正确!"));
		}
	}
...
function TreeNode(treeView, parentNode, name, text, imgSrc, tag, container, dataSource, height, isLast, hasChildren, imagePath, hasCheckBox)
{
	...
	{
		text = val;
		textDiv.innerHTML = Lesktop.Utility.ReplaceHtml(val);
	}
			...
			dataSource.GetSubNodes(
				function(subNodeData, error)
				{
					if (subNodeData != null)
					{
						...
						{
							var nd = subNodeData[i];
							var node = new TreeNode(treeView, This, nd.Name, nd.Text, nd.ImageCss, nd.Tag, subNodesContainer, dataSource, height, i == subNodeData.length - 1, nd.HasChildren, nd.ImageSrc, nd.HasCheckBox);
							subNodes.push(node);
							nodeIndex[nd.Name] = node;
						}
						if (This.IsExpand())
	...
	*/
	...
	This.GetCheckedNodes = function(out)
	{
	...
	*/
	...
	{
		dom.focus();
		if (This.IsExpand()) This.Collapse(); else This.Expand(function(result, error) { if (!result) Lesktop.Utility.ShowError(error) });
	}
	...
	textCell.innerHTML = String.format("<div class='nodeText'>{0}</div>", Lesktop.Utility.ReplaceHtml(text));
...
function ListView(config)
{
	...
	This.OnItemDblClick = new Lesktop.Delegate();

	var _columns = [];
	
	var _enableMultiSelect = Lesktop.Utility.IsNull(config.EnableMultiSelect, false);
	var _viewMode = Lesktop.Utility.IsNull(config.ViewMode, "List");
	var _list = null;
		...
		case "List":
			{
				_list = {
					Config: {
						HeaderHeight: Lesktop.Utility.IsNull(config.ListMode.HeaderHeight, 28),
						RowHeight: Lesktop.Utility.IsNull(config.ListMode.RowHeight, 24)
					}
				};
	...
	This.Reset(Lesktop.Utility.IsNull(config.Columns,[]));
	...
	function ListViewItem(values, css)
	{
		...
		dom.onmousedown = function(evt)
		{
			...
			else if (Lesktop.Utility.GetButton(evt) == "Left")
			{
				if (_enableMultiSelect)
				{
					if (evt.ctrlKey)
	...
	function ListHeader()
	{
		...
		This.OnColumnClick = new Lesktop.Delegate();
		...
		This.Create=function()
		{
			...
			for (var i in _columns)
			{
				...
				colDom.style.width = col.GetWidth() + 'px';
				width += col.GetWidth();
				colDom.className = Lesktop.Utility.IsNull(col.GetCss(),"td_header_default");
				colDom.innerHTML = String.format("<span>{0}</span>", Lesktop.Utility.ReplaceHtml(col.GetText()));
				(function(index)
				{
	...
	function ItemPanel()
	{
		...
		This.OnScroll = new Lesktop.Delegate();
...
function SingleMenu(menuConfig, parentItem, mainMenu)
{
	...
	This.Show = function(clientX, clientY)
	{
		...
		if (x + obj.offsetWidth > Lesktop.UI.PagePanel.GetWidth())
		{
			x -= obj.offsetWidth;
			if (parentItem != null) x -= parentItem.GetDom().offsetWidth;
		}
		if (y + obj.offsetHeight > Lesktop.UI.PagePanel.GetHeight())
		{
			y -= obj.offsetHeight;
...
function MenuItem(itemConfig, parent, mainMenu)
{
	...
	var obj = document.createElement("DIV");
	obj.innerHTML = Lesktop.Utility.ReplaceHtml(text);
	obj.className = css;
	Lesktop.Utility.AttachButtonEvent(obj, css, css+"_hover", css+"_hover");
...
function Frame(config)
{
	...
	{
		var e = This.GetDocument().createElement("link");
		e.rel = "StyleSheet";
		e.type = "text/css";
		e.href = Lesktop.GetUrl(href);
		var hs = This.GetDocument().getElementsByTagName("head");
		if (hs.length > 0) hs[0].appendChild(e);
		return e;
	}
	...
	This.OnBeforeLoad = new Lesktop.Delegate();
	This.OnAfterLoad = new Lesktop.Delegate();
	...
	This.SetCss(Lesktop.Utility.IsNull(config.Css, "frame"));
	This.Resize(This.GetWidth(), This.GetHeight());
...
function HtmlListBox(css, width)
{
	...
	this.AddItem=function(text,value)
	{
		...
		item_dom.innerHTML = Lesktop.Utility.ReplaceHtml(text);
...
function HtmlDropDownList(css,width,height)
{
	...
	var dom=document.createElement("DIV");
	dom.className=Lesktop.Utility.IsNull(css,"html_dropdownlist");
	...
	this.SetText=function(text)
	{
		...
		for(var i in items)
		{
			...
			{
				dom.firstChild.firstChild.innerHTML = Lesktop.Utility.ReplaceHtml(text);
				This.OnChanged.Call();
			}
		}
	}
	...
	this.SetValue=function(value)
	{
		...
		for(var i in items)
		{
			...
			{
				dom.firstChild.firstChild.innerHTML = Lesktop.Utility.ReplaceHtml(items[i].Text);
				This.OnChanged.Call();
			}
		}
	}
	...
	This.ShowList = function()
	{
		if(!m_IsListVisible)
		{
			var coord = Lesktop.Utility.GetClientCoord(dom);
			
			document.body.appendChild(listbox.GetDom());
			Lesktop.UI.PagePanel.EnterMove();
			...
			if(top + listbox.GetDom().offsetHeight > Lesktop.UI.PagePanel.GetHeight())
			{
			...
			Lesktop.Utility.AttachEvent(document, "click", document_onclick);
		}
	}
	...
	This.HideList = function()
	{
		if(m_IsListVisible)
		{
			Lesktop.UI.PagePanel.LeaveMove();
			m_IsListVisible = false;
			document.body.removeChild(listbox.GetDom());
			Lesktop.Utility.DetachEvent(document, "click", document_onclick);
		}
	}
...
function Toolbar(config)
{
	...
	This.CreateControl=function(item)
	{
		if(item.Type==undefined || item.Type=="Button")
		{
			...
			if(item.Text == "")
			{
				...
				Lesktop.Utility.AttachButtonEvent(dom, "toolbar_btn", "toolbar_btn_hover", "toolbar_btn_hover");
			}
			else
			{
				dom.className = "toolbar_text_btn";
				Lesktop.Utility.AttachButtonEvent(dom, "toolbar_text_btn", "toolbar_text_btn_hover", "toolbar_text_btn_hover");
			}
			dom.innerHTML = String.format("<div>{0}</div>", Lesktop.Utility.ReplaceHtml(item.Text), Toolbar.PredefineHeight);
	...
	{
		Lesktop.Utility.DisableSelect(This.GetDom(), true);
	}
	...
	{
		m_Btns[index].firstChild.innerHTML = Lesktop.Utility.ReplaceHtml(text);
	}
	
	This.GetButtonText=function(index)
	{
		return m_Btns[index].firstChild.innerHTML;
	}
...
function HorizSplit(config)
{
	...
	Lesktop.Utility.AttachEvent(
		...
		function(evt)
		{
			...
			Lesktop.UI.PagePanel.EnterMove("n-resize");
	...
	return new Module.HorizSplit(
		{
			...
			AnchorStyle: (top.GetAnchorStyle() & (AnchorStyle.Left | AnchorStyle.Right)) | (anchorTop ? Module.AnchorStyle.Top : Module.AnchorStyle.Bottom)
		}
	);
}
...
function VerticalSplit(config)
{
	...
	Lesktop.Utility.AttachEvent(
		...
		function(evt)
		{
			...
			Lesktop.UI.PagePanel.EnterMove("e-resize");
...
function SimpleTabControl(config)
{
	...
	This.Reset=function(tabs)
	{
		...
		{
			var tabConf = tabs[i];
			var tab = new Tab(tabConf.Text, Lesktop.Utility.IsNull(tabConf.Width, 0));
			m_Tabs.push(tab);
	...
	This.OnSelectedTab = new Lesktop.Delegate();
	...
	This.SetTabVisible = function(index, visible, autoSelect)
	{
		if(autoSelect == undefined) autoSelect = true;
		
		if(m_Tabs.length < 2) return;
		
		if(autoSelect && !visible && m_SelectedTab == m_Tabs[index])
		{
	...
	This.SetTabTitle = function(index, title)
	{		
		m_Tabs[index].SetTitle(title);
	}
	
	This.Reset(Lesktop.Utility.IsNull(config.Tabs,[]));
	...
	function Tab(text, width)
	{
		...
		var dom = document.createElement("DIV");
		dom.innerHTML = Lesktop.Utility.ReplaceHtml(text);
		...
		{
			dom.innerHTML = Lesktop.Utility.ReplaceHtml(title);
		}
		dom.onclick = function()
		{
