...
function body_onmousemove(evt)
{
	if (moveVar != null)
	{
		if (evt.clientX < 0 || evt.clientX > Lesktop.UI.PagePanel.GetWidth() || evt.clientY < 0 || evt.clientY > Lesktop.UI.PagePanel.GetHeight()) return;
...
function Window(config)
{
	...
	var baseConfig = Lesktop.Utility.Clone(config);
	...
	This.OnHided = new Lesktop.Delegate();
	This.onclosing = new Lesktop.Delegate();
	...
	This.Close = function()
	{
		Lesktop.UI.PagePanel.RemoveWindow(This);
	...
	function move(position, x, y, relativeParent)
	{
		...
		if (relativeParent && _parent != null)
			...
			rect = { Width: Lesktop.UI.PagePanel.GetWidth(), Height: Lesktop.UI.PagePanel.GetHeight(), Top: 0, Left: 0 };
	...
	if (config.Title != undefined)
	{
		...
		if (config.Title.InnerHTML != undefined) _title.InnerHTML = Lesktop.Utility.ReplaceHtml(config.Title.InnerHTML);
	}

	var m_ControlMode = Lesktop.Utility.IsNull(config.ControlMode, false);

	var _id = Lesktop.GenerateUniqueId();
	var dom = This.GetDom();
	dom.tabIndex = -1;
	...
	var m_MinHeight = Lesktop.Utility.IsNull(config.MinHeight, 300);
	var m_MinWidth = Lesktop.Utility.IsNull(config.MinWidth, 400);
	...
	var m_BkImage = new Controls.HtmlControlBkImage(
		Lesktop.Utility.IsNull(config.BkImage, { Horiz: [6, 100, 6], Vertical: [24, 100, 6], Css: "window_bk" })
	);
	...
	function maxinum()
	{
		...
		This.Resize(Lesktop.UI.PagePanel.GetClientWidth(), Lesktop.UI.PagePanel.GetClientHeight());
	...
	domTitle.childNodes[1].innerHTML = Lesktop.Utility.ReplaceHtml(_title.InnerHTML);
	...
	var _client = new Control(
		{
			...
			BorderWidth: 0,
			Parent: This,
			Css: Lesktop.Utility.IsNull(config.ClientCss, "")
		}
	);
	...
	var _waiting = new Control(
		{
			...
			BorderWidth: 0,
			Parent: This,
			Css: Lesktop.Utility.IsNull(config.Waiting, "waiting")
		}
	);
	...
	Lesktop.UI.PagePanel.AddWindow(This);
	dom = null;
	...
	This.Resize(Lesktop.Utility.IsNull(config.Width, 600), Lesktop.Utility.IsNull(config.Height, 450));
